...
## 세션 시작 프로토콜 (★ 슬라이스 작업 — `/clear` 후 필수)
이 프로젝트는 **슬라이스마다 핸드오프 + `/clear`** 로 진행한다. 새 세션에서 **코드 작업 시작 전 반드시 이 순서로 읽어라**:
1. **`.dev/handoffs/NEXT.md`** — 현재 상태 + 다음 할 일(고정 진입점). SessionStart 훅이 매 세션 이걸 가리킨다.
2. **`PLAN.md`** 의 해당 슬라이스(§7 빌드 순서 + 관련 절). 슬라이스 작업에서 PLAN.md는 옵션이 아니라 **정본**.

슬라이스 종료 시: `.dev/handoffs/slice-NN-*.md`(아카이브) 작성 + `NEXT.md`(포인터)를 다음 슬라이스로 **갱신**.
...
- 구현 설계·빌드 순서: `PLAN.md` — 단, **슬라이스 작업 시엔 "필요할 때만"이 아니라 필수**(위 "세션 시작 프로토콜").
